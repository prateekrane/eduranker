...
export default function ImagePrint({ route, navigation, autoGenerate = false }) {
    ...
    const subjectsDetected = Array.isArray(params.subjectsDetected) ? params.subjectsDetected : [];
    const sheetHeading = typeof params.sheetHeading === 'string' ? params.sheetHeading : '';
    ...
    const capture = async (showAlert = true) => {
        try {
            ...
            console.log('Students data for capture:', safeStudents.length, 'students');
            ...
            if (showAlert) {
                Alert.alert('Image generated', 'Preview below — you can download to gallery.');
            }
        } catch (err) {
    ...
    const saveToGallery = async () => {
        ...
        try {
            ...
            console.warn('Save to gallery failed, trying share fallback:', err?.message || err);
            try {
                if (await Sharing.isAvailableAsync()) {
                    await Sharing.shareAsync(capturedUri, { mimeType: 'image/png', dialogTitle: 'Save/Share Top 10 Image' });
                    Alert.alert('Share opened', 'Use the share sheet to save the image.');
                } else {
                    Alert.alert('Permission required', 'Unable to save automatically. Please allow Photos/Media permission in Settings and try again.');
                }
            } catch (e2) {
                console.error(e2);
    ...
    const generateHTML = () => {
        ...
        const studentsData = safeStudents.map((student, index) => {
            ...
            // Enhanced photo lookup with debugging
            let photo = null;
            if (id && photoMap) {
                // Try exact match first
                photo = photoMap[id];
                console.log(`Looking for photo for ID: "${id}" - Found: ${!!photo}`);

                // Try digits-only match if exact match fails
                if (!photo && idDigits) {
                    photo = photoMap[idDigits];
                    console.log(`Trying digits-only match for: "${idDigits}" - Found: ${!!photo}`);
                }
                ...
                if (!photo) {
                    ...
                    for (const [key, value] of Object.entries(photoMap)) {
                        if (key.toLowerCase() === lowerId) {
                            photo = value;
                            console.log(`Found case-insensitive match for: "${id}" -> "${key}"`);
                            break;
                        }
                    }
                }
            ...
            console.log(`Student ${student.Name} (ID: ${id}) - Photo: ${photo ? 'Found' : 'Not found'}`);
            ...
            return {
                ...
                Total: presentSubjects.length >= 2
                    ? presentSubjects.reduce((sum, subject) =>
                        sum + (student.Subjects?.[subject] || 0), 0)
                    : undefined,
                photo,
            };
        });
        ...
        const generateStudentCells = (student, bgColor) => {
            ...
            // Photo cell with enhanced error handling
            let photoHtml = '';
            if (student.name !== '-') {
                if (student.photo) {
                    // Add error handling for broken images
                    photoHtml = `<img src="${student.photo}" alt="${student.name}" style="width: 56px; height: 56px; border-radius: 8px; object-fit: cover;" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';" />
                        <div style="display: none; width: 56px; height: 56px; border-radius: 8px; background: #3498db; color: white; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: bold;">${initials(student.name)}</div>`;
                } else {
                    photoHtml = `<img src="https://placehold.co/100x100/3498db/ffffff?text=${encodeURIComponent(initials(student.name))}" alt="${student.name}" style="width: 56px; height: 56px; border-radius: 8px; object-fit: cover;"/>`;
                }
            }
            cells.push(`<td class="${bgColor} col-photo text-center">${photoHtml}</td>`);

            // Rank and Name
            cells.push(`<td class="${bgColor} col-rank text-center font-bold text-black">${student.rank || ''}</td>`);
            cells.push(`<td class="${bgColor} col-name text-left font-semibold text-black">${student.name || ''}</td>`);

            // Subject marks
            presentSubjects.forEach(subject => {
                cells.push(`<td class="bg-sky-300 col-subject text-center font-extrabold text-black">${student.Subjects[subject] !== undefined ? student.Subjects[subject] : ''
                    }</td>`);
            });

            // Total (only if 2 or more subjects)
            if (presentSubjects.length >= 2) {
                cells.push(`<td class="bg-sky-300 col-total text-center font-extrabold text-black">${student.Total !== undefined ? student.Total : ''
                    }</td>`);
            }
        ...
        for (let i = 0; i < studentsData.length; i += 2) {
            ...
            const right = studentsData[i + 1] || { rank: '', name: '-', Subjects: {} };
            const leftColor = (Math.floor(i / 2) % 2 === 0) ? 'bg-lime-200' : 'bg-sky-200';
            const rightColor = (Math.floor(i / 2) % 2 === 0) ? 'bg-sky-200' : 'bg-lime-200';
        ...
        <div style="width: 1200px; height: 800px; margin: 0 auto;">
            <header class="text-center">
                <div class="bg-red-600 text-white font-black text-xl sm:text-2xl md:text-3xl py-3 shadow-lg text-shadow-heavy tracking-wide" style="text-align: center;">
                    ${sheetHeading ? sheetHeading.replace(/</g, '&lt;').replace(/>/g, '&gt;') : 'TOPPER\'S LIST'}
                </div>
                ${sheetHeading ? '' : `<div class="bg-blue-900 py-2 shadow-md" style="text-align: center;"><h2 class="text-white font-bold text-lg sm:text-xl md:text-2xl">CLUSTER TEST</h2></div>`}
            </header>
    ...
    useEffect(() => {
        ...
        console.log('Students data:', safeStudents);
        console.log('Photo map keys:', Object.keys(photoMap || {}));
        console.log('Photo map sample entries:', Object.entries(photoMap || {}).slice(0, 3));
        ...
        if (safeStudents.length > 0 && photoMap) {
            ...
            safeStudents.slice(0, 3).forEach((student, index) => {
                ...
                if (photoMap[id]) console.log(`  - Photo URL: ${photoMap[id].substring(0, 50)}...`);
            });
        }
    ...
    const html = useMemo(() => {
        ...
        console.log('HTML preview (first 500 chars):', generatedHtml.substring(0, 500));
        return generatedHtml;
    }, [safeStudents]);

    return (
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingVertical: 20, backgroundColor: '#f0f0f0' }}>
            {/* Hidden WebView for generation only */}
            <ViewShot ref={viewRef} options={{ format: 'png', quality: 1.0, width: 1200, height: 800 }} style={{ width: 1200, height: 800, position: 'absolute', left: -10000, top: -10000, opacity: 0, pointerEvents: 'none' }}>
                <View style={[styles.webViewContainer, { width: 1200, height: 800 }]}>
                    <WebView
                        ...
                        onLoadStart={() => { console.log('WebView load started'); setIsGenerating(true); }}
                        onLoadEnd={() => {
                            console.log('WebView load ended');
                            if (autoGenerate && !capturedUri && !hasCaptured) {
                                console.log('Triggering auto-capture after WebView load');
                                setTimeout(() => {
                                    try { capture(false); } catch (e) { console.error('Auto-capture failed:', e); }
                                }, 1000);
                            }
                        }}
            ...
            {isGenerating && !capturedUri ? (
                ...
                <Text style={{ color: '#666', fontStyle: 'italic' }}>No image available</Text>
            )}
        </View>
    );
}
