...
import { View, Text, TouchableOpacity, ScrollView, StyleSheet, Alert, ActivityIndicator, Image, Modal, Dimensions } from 'react-native';
...
const BackgroundSVG = () => (
    <Svg width="100%" height="100%" style={StyleSheet.absoluteFill}>
        ...
        <Circle cx={width * 0.15} cy={height * 0.2} r="56" fill="rgba(167,139,250,0.12)" />
        <Circle cx={width * 0.85} cy={height * 0.3} r="36" fill="rgba(124,58,237,0.10)" />
        <Circle cx={width * 0.75} cy={height * 0.8} r="70" fill="rgba(167,139,250,0.08)" />
    </Svg>
);
...
export default function Result({ navigation }) {
    ...
    const importExcel = async () => {
        try {
            ...
            const extractHeading = (ws) => {
                if (!ws) return '';
                // Prefer A1 if present and string
                if (ws['A1'] && typeof ws['A1'].v === 'string' && ws['A1'].v.trim()) {
                    return ws['A1'].v.toString().trim();
                }
                // Scan first 3 rows, columns A..M for first non-empty longish string
                const cols = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.slice(0, 13).split('');
                for (let r = 1; r <= 3; r++) {
                    for (let c = 0; c < cols.length; c++) {
                        ...
                        if (cell && typeof cell.v !== 'undefined') {
                            ...
                            if (val && val.length >= 4 && /[A-Za-z]/.test(val)) {
                                return val;
                            }
                        }
                    }
                }
                // Try merged cells covering top row
                const merges = ws['!merges'] || [];
                for (const m of merges) {
                    if (m.s.r === 0) { // starts on row 1
            ...
            const normalizedRows = Array.isArray(json) ? json.map(normalizeObjKeys) : [];
            ...
            setFilePicked(true);
            Alert.alert('Imported', `Found ${normalizedRows.length} rows. Ready to get result.${heading ? `\nHeading: ${heading}` : ''}`);
        } catch (err) {
            console.error('importExcel error:', err);
            Alert.alert('Error', 'Failed to import Excel file. Make sure it is a valid .xlsx or .xls file.');
        } finally {
    ...
    const importMasterExcel = async () => {
        try {
            ...
            if (typeof res === 'object' && 'canceled' in res) {
                ...
                fileUri = asset.uri;
                try { Alert.alert('File selected', asset.name || fileUri); } catch { }
            } else {
                if (res.type !== 'success') return;
                fileUri = res.uri;
                try { Alert.alert('File selected', res.name || fileUri); } catch { }
            }
            ...
            console.log('DEBUG - Master Excel header keys:', Object.keys(rows[0] || {}));
            ...
            keys.forEach((k) => {
                ...
                if (!rollKey && (norm === 'candidateid' || norm === 'rollno' || norm === 'rollnumber')) rollKey = k;
                if (!imageKey && norm === 'photo') imageKey = k;
            });
            // fallback: try to match with includes if not found yet
            if (!rollKey) {
                for (const k of keys) {
                    if (normalize(k.trim()).includes('candidateid') || normalize(k.trim()).includes('rollno')) {
                        rollKey = k; break;
                    }
                }
            }
            if (!imageKey) {
                for (const k of keys) {
                    if (normalize(k.trim()) === 'photo') { imageKey = k; break; }
                    if (normalize(k.trim()).includes('photo')) { imageKey = k; break; }
                }
            }
            ...
            if (!rollKey) {
                ...
                const candidate = keys.find(k => /(roll|candidate\s*id|cand\s*id)/.test(k.toLowerCase()));
                if (candidate) rollKey = candidate;
            }

            if (!rollKey) {
                Alert.alert('Missing column', 'Could not detect "Roll No" column in Master sheet.');
                return;
            }
            ...
            const normalizePhoto = (val) => {
                ...
                const b64Like = /^[A-Za-z0-9+/=\r\n]+$/.test(raw) && raw.replace(/\s+/g, '').length > 100;
                if (b64Like) {
            ...
            rows.forEach((r, index) => {
                ...
                const img = imageKey ? normalizePhoto(r[imageKey]) : '';
                if (typeof img === 'string' && img.length > 0) {
                    // also store a digits-only version for robust matching
                    const digits = roll.replace(/\D+/g, '');
                    map[roll] = img; // original
                    if (digits && !map[digits]) map[digits] = img;
                    photoCount++;

                    // Log first few entries for debugging
                    if (index < 3) {
                        console.log(`Photo mapping ${index + 1}: ID="${roll}" -> Photo found (${img.length} chars)`);
                        if (digits && digits !== roll) {
                            console.log(`  Also mapped digits: "${digits}"`);
                        }
                    }
                } else if (index < 3) {
                    console.log(`No photo for ID: "${roll}" (imageKey: ${imageKey}, value: ${r[imageKey]})`);
                }
            });

            console.log(`Built photo map with ${photoCount} photos from ${rows.length} rows`);

            setMasterRows(rows);
            setPhotoMap(map);
            // DEBUG LOGGING for map content
            ...
            Object.entries(map).slice(0, 10).forEach(([id, val], idx) => console.log(`DEBUG - Map[${id}]: ${typeof val} - ${val && val.slice ? val.slice(0, 100) : val}`));
            setMasterPicked(true);
            Alert.alert('Master Imported', `Found ${rows.length} rows${Object.keys(map).length ? `, ${Object.keys(map).length} photos mapped` : ''}.`);
        } catch (err) {
    ...
    const removeMasterAndClear = async () => {
        if (loadingMaster || loadingMarks || clearingMasterCache || clearingMarksCache) return;
        try {
            ...
            Alert.alert('Cleared', `Master removed. Cache cleared${deleted ? ` (${deleted} items)` : ''}.`);
        } catch (err) {
    ...
    const removeMarksAndClear = async () => {
        ...
        try {
            ...
            if (typeof setExtractedData === 'function') {
                try { setExtractedData([]); } catch { }
            }
            const deleted = await clearAppCache(setClearingMarksCache);
            Alert.alert('Cleared', `Marks removed. Cache cleared${deleted ? ` (${deleted} items)` : ''}.`);
        } catch (err) {
    ...
    const detectColumnsAndTop10 = (rows) => {
        ...
        if (rows[0] && Object.values(rows[0]).some(v =>
            typeof v === 'string' &&
        ...
        const keyStats = {};
        keys.forEach(k => {
            const values = sample.map(r => r[k]).filter(v => v !== undefined && v !== null && v !== '');
            ...
            const avgLen = strVals.length ? (strVals.reduce((a, b) => a + b.toString().length, 0) / strVals.length) : 0;
            const spaceRatio = strVals.length ? (strVals.filter(s => /\s/.test(s)).length / strVals.length) : 0;
        ...
        const isIdLike = (k) => !!(keyStats[k] && (keyStats[k].avg > 1000 || keyStats[k].max > 1000));
        ...
        const isMarksLike = (k) => {
            ...
            const sampleValues = sample.map(r => r[k]).filter(v => v !== undefined && v !== null && v !== '');
            const strVals = sampleValues.filter(v => typeof v === 'string');
            const labelKeywords = ['group', 'jee', 'neet', 'batch', 'section', 'sec a', 'int sec'];
            const labely = strVals.filter(s => labelKeywords.some(w => String(s).toLowerCase().includes(w)));
        ...
        const numericCandidates = keys.filter(k => keyStats[k] && keyStats[k].numRatio > 0.8 && k !== idKey);
        const pickBestMarksColumn = () => {
            // Prefer common placement then best stats
            const preferredOrder = [
                '__empty_2', '__empty', '__empty_1'
            ];
        ...
        console.log('Detected columns - ID:', idKey, 'Name:', nameKey, 'Subjects map:', subjectKeyMap);
        console.log('Keys stats for debugging:', Object.keys(keyStats).map(k => ({ key: k, ...keyStats[k] })));
        ...
        if (headerRow) {
            ...
            console.log('Skipping header row, processing', dataRows.length, 'data rows');
        }
        ...
        const mapped = dataRows.map(r => {
            try {
                ...
                if (Object.values(r).some(v =>
                    typeof v === 'string' &&
                    ['candidate id', 'candidate name', 'group', 'phy', 'chem', 'maths', 'total'].includes(v.toString().toLowerCase())
                )) {
                    return null;
                }
                ...
                if (!nameVal || isNumericLike(nameVal)) {
                    ...
                    for (const k of altKeys) {
                        if (k in r && r[k] && !isNumericLike(r[k])) { nameVal = r[k]; break; }
                    }
                }
                ...
                subjectsDetected.forEach(s => {
                    ...
                    if (!value) {
                        const subjectKeys = Object.keys(r).filter(k => {
                            ...
                            return s.patterns.some(p => p.test(kl)) ||
        ...
        console.log('Students with ranks assigned:', mapped.slice(0, 10).map(s => ({
            name: s.Name,
            total: s.Total,
            rank: s.Rank,
            subjects: s.Subjects
        ...
        return Array.isArray(result)
            ? { students: result, allStudents: mapped, subjectsDetected: subjectsDetected.map(s => s.label) }
    ...
    const getResult = () => {
        ...
        if (!masterPicked) {
            ...
            Alert.alert('Optional: Master Excel', 'Master Excel not imported. Photos may not be shown.');
        }
        try {
            ...
            const subjectsDetected = (topRes && topRes.subjectsDetected) ? topRes.subjectsDetected : [];

            if (!top10.length) {
                Alert.alert('No data', 'Could not find suitable columns (ID, Name, Marks). Please check the Excel format.');
                return;
            }
        ...
        <SafeAreaView style={styles.safeArea}>
            ...
            <ScrollView style={styles.scroll} contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
                <View style={styles.headerWrap}>
                    ...
                    <Text style={styles.subheader}>Import your Excel file and generate the Top 10 automatically</Text>
                </View>

                {/* Master Excel first */}
                <TouchableOpacity style={[styles.button, (loadingMaster || loadingMarks) && { opacity: 0.6 }]} onPress={importMasterExcel} disabled={loadingMaster || loadingMarks}>
                    <ExpoLinearGradient colors={["#2563EB", "#60A5FA"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.buttonGradient}>
                        <Ionicons name="person-circle-outline" size={18} color="#fff" style={{ marginRight: 8 }} />
                        <Text style={styles.buttonText}>{loadingMaster ? 'Importing Master...' : 'Import Master Excel'}</Text>
                    </ExpoLinearGradient>
                </TouchableOpacity>

                {/* Remove Master + Clear Cache */}
                <TouchableOpacity
                    style={[styles.button, (!masterPicked || loadingMaster || loadingMarks || clearingMasterCache || clearingMarksCache) && { opacity: 0.6, }]}
                    onPress={removeMasterAndClear}
                    disabled={!masterPicked || loadingMaster || loadingMarks || clearingMasterCache || clearingMarksCache}
                >
                    <ExpoLinearGradient colors={["#EF4444", "#F87171"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.buttonGradient}>
                        <Ionicons name="trash-outline" size={18} color="#fff" style={{ marginRight: 8 }} />
                        <Text style={styles.buttonText}>{clearingMasterCache ? 'Clearing Cache...' : 'Remove Master & Clear Cache'}</Text>
                    </ExpoLinearGradient>
                </TouchableOpacity>

                {/* Master status chip */}
                <View style={styles.statusRow}>
                    <View style={[styles.chip, masterPicked ? styles.chipSuccess : styles.chipPending]}>
                        <Ionicons name={masterPicked ? 'checkmark-circle' : 'alert-circle'} size={14} color="#fff" style={{ marginRight: 6 }} />
                        <Text style={styles.chipText}>{masterPicked ? `Master: Uploaded (${masterRows.length} rows${Object.keys(photoMap || {}).length ? `, ${Object.keys(photoMap).length} photos` : ''})` : 'Master: Not uploaded'}</Text>
                    </View>
                </View>

                {/* Marks Excel second */}
                <TouchableOpacity style={[styles.button, (loadingMaster || loadingMarks) && { opacity: 0.6 }]} onPress={importExcel} disabled={loadingMaster || loadingMarks}>
                    <ExpoLinearGradient colors={["#7C3AED", "#A78BFA"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.buttonGradient}>
                        ...
                        <Text style={styles.buttonText}>{loadingMarks ? 'Importing Marks...' : 'Import Marks Excel'}</Text>
                    </ExpoLinearGradient>
                </TouchableOpacity>

                {/* Remove Marks + Clear Cache */}
                <TouchableOpacity
                    style={[styles.button, (!filePicked || loadingMaster || loadingMarks || clearingMasterCache || clearingMarksCache) && { opacity: 0.6, }]}
                    onPress={removeMarksAndClear}
                    disabled={!filePicked || loadingMaster || loadingMarks || clearingMasterCache || clearingMarksCache}
                >
                    <ExpoLinearGradient colors={["#DC2626", "#EF4444"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.buttonGradient}>
                        <Ionicons name="trash-bin-outline" size={18} color="#fff" style={{ marginRight: 8 }} />
                        <Text style={styles.buttonText}>{clearingMarksCache ? 'Clearing Cache...' : 'Remove Marks & Clear Cache'}</Text>
                    </ExpoLinearGradient>
                </TouchableOpacity>

                {/* Marks status chip */}
                <View style={styles.statusRow}>
                    <View style={[styles.chip, filePicked ? styles.chipSuccess : styles.chipPending]}>
                        <Ionicons name={filePicked ? 'checkmark-circle' : 'alert-circle'} size={14} color="#fff" style={{ marginRight: 6 }} />
                        <Text style={styles.chipText}>{filePicked ? `Marks: Uploaded (${rawData.length} rows)` : 'Marks: Not uploaded'}</Text>
                    </View>
                </View>

                <TouchableOpacity style={[styles.button, (!filePicked || loadingMaster || loadingMarks) && { opacity: 0.6 }]} disabled={!filePicked || loadingMaster || loadingMarks} onPress={getResult}>
                    <ExpoLinearGradient colors={["#10B981", "#34D399"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.buttonGradient}>
                        <Ionicons name="trophy-outline" size={18} color="#fff" style={{ marginRight: 8 }} />
                        <Text style={styles.buttonText}>Get Result</Text>
                    </ExpoLinearGradient>
                </TouchableOpacity>

                <View style={styles.noteCard}>
                    <Ionicons name="information-circle-outline" size={18} color="#C4B5FD" style={{ marginRight: 8 }} />
                    <Text style={styles.hint}>The app will auto-detect Candidate ID, Student Name, and Marks. Master Excel is used to fetch photos by matching Roll No to Candidate ID.</Text>
                </View>

                {/* Data Preview Section */}
                {showDataPreview && top10Data.length > 0 && (
                    <View style={styles.previewContainer}>
                        <View className="dataPreviewHeader" style={styles.dataPreviewHeader}>
                ...
                (loadingMaster || loadingMarks) && (
                    <View style={styles.loadingOverlay}>
                        <View style={styles.loadingCard}>
                            ...
                            <Text style={styles.loadingText}>{loadingMaster ? 'Importing Master sheet...' : 'Importing Marks sheet...'}</Text>
                        </View>
                    </View>
                )
            }
